...
class Exam(QMainWindow, form_window):
    def __init__(self):
        ...
        self.model = load_model('../model/human_horse_classification.h5')
    ...
    def predict_image(self):
        ...
        self.path = QFileDialog.getOpenFileName(
            self, "Open file",'../datasets/horse-or-human',
            "Image Files(*.png);;All Files(*.*);;JPG Files(*.jpg)",
        )
        print(self.path)
        ...
        if self.path[0]:
            ...
            if predict_value >0.5:
                self.lbl_predict.setText('이 이미지는 ' + str((predict_value*100).round()) + '% 확률로 말 입니다.')
            else:
                self.lbl_predict.setText('이 이미지는 ' + str(((1-predict_value)*100).round()) + '% 확률로 사람 입니다.')
